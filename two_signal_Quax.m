%Program to simulate new two-signal Kang/Quax model
%The original plan was to implement as described below - but it was decided to
%implement a simpler version because of the limitations described in the
%PLOS CB paper.

% New model: There are two cortical layers and two regions (say H- and V-tuned)
% within each layer, all modeled by EI pairs oscillating (quasi-cycles) at 40 Hz.
% Cortical regions in layer 2 receive inpts from the same region in layer 1
% with a delay of 125 ms - constant phase offset of about -\pi/2.
...
% down signals so as to create a larger input response from either of the
% two cortical regions (tuned to one of the two inputs - or signals). 

% The above was not implemented in the final sims for the PLoS CB paper.
% Instead a simpler version that just simulated a single cortical stream
% (V1 and V2) with noisy sine wave alpha input from a putative pulvinar
% nucleus was implemented. In this simulation making alpha inputs zero
% implements inhibition of the pulvinar by the dominant stream as 
% described in the even newer model described in the PLOS CB paper. This is
% done in a separate run rather than being done in realtime. There is still
% some code from the original version (most commented out) left in this program;
% it can be ignored.
...
mxV1E=zeros(nits,1);        %variables for iterated sigoe etc
mnV1E=zeros(nits,1);
mxV2E=zeros(nits,1);
mnV2E=zeros(nits,1);

sig1E=0.15.*ones(N,1);       %cortical noise base intensities
sig1I=0.15.*ones(N,1);
sig2E=0.15.*ones(N,1);
sig2I=0.15.*ones(N,1);

amag=2;                     % initial amplitude of noisy 10 Hz sine waves
                            %multiplied by alphafact to get final alpha
                            %amplitude
alphafact1=3;                 %multiplier of pulvinar alpha from VPE1 or alpha1 to V1I
alphafact2=3;             %multiplier of pulvinar alpha from VPE2 or alpha2 to V2I
                            %total alpha mag = alphafact.*amag

ialpha=1;            %top-down alpha trigger=0 if no alpha to cortex
%alagindex=123;          %>lag of second quasi-alpha generator
%alag=120;              %lag of second quasi-alpha in msec=alag*delt
tdtrn=-0.0;              %input to TRN that decreases PI processes (in pulvinar)
P2IP1I=0.0;             %input to VP2I from VP1I if quasi-alpha

nsesig=0.0;                %noise in signal
sigmag=20.0;                %signal magnitude
signal(1,1:50000)=0;        %signal = 0 before timepoint 50000
signal(1,50001:runtime)=sigmag+(nsesig.*randn(N,50000));  %signal after 50000-noisy if nsesig>0

S1E2E=0.5.*ones(N,1);      %input from V1E to V2E: cortical connectivity


determ=1;                   % determ=1 for noisy 10 Hz sine waves
                            % determ=0 for EI pair 10 Hz quasi-cycles

V2Eoff=1;             % delay to adjust phase offset between cortical 
                        % layers 1 and 2 - same for both input signals
                        % 125 timepoints = 6.25 msec = -\pi/2 radians at 40 Hz
                        % V2Eoff=1 necessary to get 0 delay re programming
...
noV2=0;             %=1 if no V2 quasi-cycles - only noise
nV2nse=0.02;         %noise for noV2 processes
    
    for jj=1:nits          % iteration loop
      
      if determ==1            %noisy sine wave alpha input to cortex
      ...
      else     %alpha quasi-cycles input to cortex
    ...
    cornse=0.20;                            %cortical noise multiplier times base noise in SDE
    noiseE1=cornse.*randn(N,runtime);      %independent noises - same for H and V
    noiseI1=cornse.*randn(N,runtime);      % layer 1
    noiseE2=cornse.*randn(N,runtime);      %independent noises - same for H and V
    noiseI2=cornse.*randn(N,runtime);      % layer 2
...
for k=2:runtime      %basic loop
    ...
    %now compute voltages - solve SDE using Euler-Maruyama
        %EI pulvinar
        if k>sigon && k<sigoff
            dV1E(:,k)=((delt.*(-V1E(:,k-1)+(S1EE(:,1).*V1E(:,k-1))-(S1EI(:,1).*V1I(:,k-1))+signal(:,k)))+(sig1E(:,1).*sqrtdelt.*noiseE1(:,k)))./tauE;
        else
    ...
    else      %determ=1: 10 Hz from noisy sin waves to cortex
...
B=mean(V1E(1,55000:runtime));
C=mean(V2E(1,55000:runtime));
BB=mean(mxV1E);
CC=mean(mxV2E);
BBB=BB-B;
CCC=CC-C;
